# ==============================================================================
# src/screens/forms/form4_detail.py — หน้ารายละเอียดฟอร์ม 4
# ==============================================================================
# แบบขอหนังสือเชิญเป็นผู้ทรงคุณวุฒิ ตรวจและประเมิน...เพื่อวิจัย
# ข้อมูลที่แสดง: ข้อมูลนักศึกษา + หัวข้อวิทยานิพนธ์ + ข้อมูลผู้ทรงคุณวุฒิ
# Route: "/form4/{submission_id}"
# ==============================================================================
import flet as ft
...
def FormFourDetailScreen(page: ft.Page, submission_id: str):
    """สร้างหน้าจอรายละเอียด Form 4 (ขอเชิญผู้ทรงคุณวุฒิ)"""
    controller = FormController()

    user_id = require_auth(page)
    if not user_id:
        return ft.View(route=f"/form4/{submission_id}", controls=[ft.Text("Redirecting to login...")])
    ...
    # ตัวแปรสำหรับข้อมูลผู้ทรงคุณวุฒิ
    expert_title_val = form_text_value()    # คำนำหน้า/ยศ
    expert_name_val = form_text_value()     # ชื่อ
    expert_surname_val = form_text_value()  # นามสกุล
    expert_org_val = form_text_value()      # สถาบัน/หน่วยงาน
    expert_phone_val = form_text_value()    # เบอร์โทร
    expert_email_val = form_text_value()    # อีเมล
    ...
    # การ์ดข้อมูลผู้ทรงคุณวุฒิ (เฉพาะ Form 4)
    expert_card = FormDetailCard(
        content=ft.Column(spacing=12, controls=[
            ft.Text("ข้อมูลผู้ทรงคุณวุฒิ คนที่ 1", size=16, weight="bold", color=APP_COLORS["black"]),
            ft.Divider(height=10, color="transparent"),
    ...
    return ft.View(
        ...
        controls=[ft.Column(controls=[
            ft.Container(
                ...
                content=ft.Text("แบบขอหนังสือเชิญเป็นผู้ทรงคุณวุฒิ\nตรวจและประเมิน...เพื่อวิจัย", size=18, weight="bold", color=APP_COLORS["black"], text_align="center"),
                alignment=ft.alignment.center,
            ),
            student_card, ft.Container(padding=5), thesis_card, ft.Container(padding=5), expert_card, ft.Container(padding=20),
        ])],
    )
